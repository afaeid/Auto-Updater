...
const presentAction = (actionHist, config)=>{
    ...
    if(actionHist.created > 0 || actionHist.updated > 0 || actionHist.deleted > 0){
      ...
      console.log(chalk.bgGreenBright.bold(`Total ${actionHist.created + actionHist.updated + actionHist.deleted} in ${((Date.now() - process.startTime)/1000).toFixed(2)} second(s)`))
